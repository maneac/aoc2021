...
type Input = [RangeInclusive<isize>; 2];
...
fn part_2(input: &Input) -> usize {
    ...
    // => x(x+1)/2 = min x
    let min_x = (((1.0 + (8.0 * *input[0].start() as f64)).sqrt() - 1.0) / 2.0).ceil() as isize;

    let max_y = find_max_y(input) as isize;

    let valid_xs = (min_x..=*input[0].end())
        .filter(|x| can_x_achieve_range(input, x))
        .collect::<Vec<isize>>();

    let valid_ys = (*input[1].start()..=max_y)
    ...
    valid_xs
        ...
        .filter(|&(x, y)| {
            ...
            loop {
                s_x += v_x;
                s_y += v_y;
                if input[0].contains(&s_x) && input[1].contains(&s_y) {
                    return true;
                }
                if s_x.gt(input[0].end()) || s_y.lt(input[1].start()) {
                    return false;
                }
                v_x -= v_x.signum();
                v_y -= 1;
...
fn find_max_y(input: &Input) -> usize {
    for y in (0..-input[1].start()).rev() {
        let mut v_y = -(y + 1);
        let mut y_sum = 0;
        while y_sum.ge(input[1].start()) {
            y_sum += v_y;
            v_y -= 1;
            if input[1].contains(&y_sum) {
                return y as usize;
            }
        }
    }
...
fn can_x_achieve_range(input: &Input, x: &isize) -> bool {
    ...
    loop {
        s_x += v_x;
        if input[0].contains(&s_x) {
            return true;
        }
        if v_x == 0 || s_x.gt(input[0].end()) {
            return false;
        }
        v_x -= v_x.signum();
    }
...
fn can_y_achieve_range(input: &Input, y: &isize) -> bool {
    ...
    loop {
        s_y += v_y;
        if input[1].contains(&s_y) {
            return true;
        }
        if s_y.lt(input[1].start()) {
            return false;
        }
        v_y -= 1;
    }
...
mod tests {
    ...
    const PART_1: usize = 12090;
    const PART_2: usize = 5059;
