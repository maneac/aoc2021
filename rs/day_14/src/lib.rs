...
pub const PART_1: u64 = 2584;
pub const PART_2: u64 = 3816397135460;
...
impl PartialOrd for Input {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        ...
        match self.last.partial_cmp(&other.last) {
            Some(core::cmp::Ordering::Equal) => {}
            ord => return ord,
        }
        match self.initial.keys().partial_cmp(other.initial.keys()) {
            Some(core::cmp::Ordering::Equal) => {}
            ord => return ord,
        }
        self.insertions.keys().partial_cmp(other.insertions.keys())
    }
}
