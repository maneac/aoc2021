...
fn part_2(input: &Input) -> usize {
    ...
    for (lhs_idx, lhs) in input.scanner_locations.iter().enumerate() {
        for rhs in input.scanner_locations.iter().skip(lhs_idx + 1) {
            let beacon_distance =
                (lhs.x.abs_diff(rhs.x)) + (lhs.y.abs_diff(rhs.y)) + (lhs.z.abs_diff(rhs.z));
            max_distance = max_distance.max(beacon_distance);
        }
    }
    max_distance
}
...
fn fold_input(input: &[Vec<Point>], order: Vec<usize>, threshold: usize) -> Input {
    ...
    Input {
        beacons,
        scanner_locations: translations,
    }
}
...
// fn transform_points(beacons: &mut Vec<Point>, facing: u8) {
//     core::arch::x86_64::
// }
...
mod tests {
    ...
    const PART_1: usize = 483;
    const PART_2: usize = 14804;
